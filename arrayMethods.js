...
const sixteenthCentury = inventors.filter((inventor) => {
  return inventor.year > 1499 && inventor.year < 1600;
});
console.log("Geboren in de zestiende eeuw: ")
console.log(sixteenthCentury);
...
const birthDate = inventors.map((inventor) => {
  return inventor.year;
});

console.log("Geboorte jaar: " + birthDate);
...
const fullName = inventors.map(investor => {
  return investor.first + " " + investor.last;
})

console.log("Uitvinders: " + fullName)
...
const findName = inventors.find((inventor)=>{
